...
class _DroneControlPageState extends State<DroneControlPage> {
  ...
  Future<void> _connectSocket() async {
    // Reutilizamos el socket inicializado en la waiting room
    ...
    _socket!
      ...
      ..onConnectError((err) => print('connect_error: $err'))
  ...
  void _rebuildMarker(String email) {
    ...
    final opacity = state == 'landed' ? (_blinkVisible ? 1.0 : 0.0) : 1.0;
  ...
  void _addFence(dynamic geometry, String droneEmail) {
    ...
    final points = geometry.map<LatLng>((pt) => LatLng(pt['lat'] as double, pt['lon'] as double)).toList();
  ...
  Widget _joystick(String id) => Joystick(
    ...
    base: Container(
      width: 120,
      height: 120,
      decoration: const BoxDecoration(color: Color(0xFF2F3B4C), shape: BoxShape.circle),
    ),
    stick: Container(
      width: 60,
      height: 60,
